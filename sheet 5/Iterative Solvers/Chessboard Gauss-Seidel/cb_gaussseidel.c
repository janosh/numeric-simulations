...
void cb_gs_step(double *phi, double *rho, int N, double h, double G)
{
    // chess-board iteration scheme (this for-loop is the only difference to the regular Gauss-Seidel scheme)
    for(int k = 0; k<2; k++)
    for(int i = 0; i < N; i++)
    for(int j = (((i & 1) == k) ? 1 : 0); j < N; j += 2)
    {
...
int main(int argc, char **argv)
{
    ...
    FILE *datafile = fopen("res_gausseidel_cb.txt", "w");

    printf("\nChessboard Gauss-Seidel\n");
    for(int i = 0; i <= Nsteps; i++)
    {
        cb_gs_step(phi, rho, N, h, G);
